...
export const UserCart = ({
    ...
    const { data: cartData, isLoading: cartLoading } = useQuery({
        ...
        queryFn: async (): Promise<CartResponse> => {
            ...
            if (authError || !user) throw new Error("AUTH_ERROR: Giriş yapmanız gerekmektedir.");
        ...
        staleTime: 2 * 60 * 1000,
        gcTime: 5 * 60 * 1000,
        retry: (failureCount, error) => error.message.startsWith('AUTH_ERROR:') ? false : failureCount < 1,
    ...
    const removeCartItemMutation = useMutation({
        ...
        onError: () => showAlertMessage?.("Ürün sepetten kaldırılamadı.")
    });
    ...
    const updateQuantityMutation = useMutation({
        ...
        onError: () => showAlertMessage?.("Adet güncellenemedi.")
    });

    const handleRemoveFromCart = useCallback((cart_item_id: string) => {
        setItemLoading(cart_item_id);
        removeCartItemMutation.mutate(cart_item_id, { onSettled: () => setItemLoading(null) });
    }, [removeCartItemMutation]);

    const handleUpdateQuantity = useCallback((cart_item_id: string, newQuantity: number) => {
        if (newQuantity < 1) return;
        setItemLoading(cart_item_id);
        updateQuantityMutation.mutate({ cartItemId: cart_item_id, quantity: newQuantity }, { onSettled: () => setItemLoading(null) });
        ...
        return (
            <Card>
                <CardContent className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">Sepetinize ürün eklediğinizde burada görebilirsiniz.</p>
                </CardContent>
            </Card>
        );
    }
                        ...
                        <Card className="top-4 flex-1">
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex justify-between">
                                    ...
                                    <span>{formatPrice(cartData.summary.subtotal)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-muted-foreground">KDV (20%)</span>
                                    <span>{formatPrice(cartData.summary.estimated_tax)}</span>
                                </div>
                                <div className="flex justify-between">
                                    ...
                                    <span>
                                        {cartData.summary.shipping_cost === 0
                                            ...
                                            : formatPrice(cartData.summary.shipping_cost)}
                                    </span>
                                </div>
                                {cartData.summary.subtotal < 50 && cartData.summary.shipping_cost > 0 && (
                                    <div className="text-xs text-muted-foreground bg-blue-50 p-2 rounded">
                                        💡 Ücretsiz kargo için {formatPrice(50 - cartData.summary.subtotal)} daha ekleyin!
                                    </div>
                                )}
                                <hr />
